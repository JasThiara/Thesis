...
\begin{itemize}
  \item Identify other means to detect or determine asset bubble.
\end{itemize}
\cite{FAPVBPS} paper covers variance bounds testing and malformed time series as a method of detection and
detection difficulty respectively. \cite{MTFBIHM} panel data test and cointegration are used to detect bubbles
in housing prices.\cite{bigBubble} RKHS theory is used to detect asset bubbles.\cite{GBAPB} used a bootrapping
methodology to indicate type 3 bubbles in the Nasdaq price index and Case-Shiller house price index.
\cite{YAFEAISMB} use a Kalman filter to estimate a bubble and then use a state space model with Markov switching
to model the survivability of a bubble, switching between collapsing and surving states.
