...
\begin{enumerate}
  \item Objective portfolio theory to minimize risk given.
  \item An expected return on assets\cite{MPS}.
  \item Key definations.
  \begin{itemize}
    \item Portfolio has set of asset in manners which minimize the risks.
    \item Primary way to manage portfolio with the aim of distributing risk.
    \item Market Price- current price
    \item Fundamental Price- Intrisic price.
    \item The bubble is the difference between market and fundamental price. 
    \item The fluctuation of asset price is volatillity od asset.
  \end{itemize}
  \item Our thesis problem consists detecting type 3 bubble within market price\cite{LHM}
\end{enumerate}
\section{Mathematical Background of Portfolio Theory}
\begin{enumerate}
  \item Random Variable \cite{AIAA}
  \item \cite{BIIE}
  \begin{itemize}
    \item moment Generator function of random variable.
    \item Expected value of an random variable.
    \item Variance of a random variable
  \end{itemize}
  \item Sigma Albegra\cite{CMT}
  \item \cite{GHHM}
  \begin{itemize}
    ...
    \item Martingale
  \end{itemize}
  \item fundamential pricesa stochastic process\cite{HAAE,OSSE}
  \item Market Price as a stochastic process\cite{HAAE,OSSE}
  \item Asset Bubble\cite{FAPVBPS}
  \item Volatility\cite{FAPVBPS}
  \item SDE\cite{GIIE}
  \item Euler Marayama\cite{HAAE}
  \item Floren Zmirou\cite{FZOFTD}
  \item Theorem strict local martingale iff \cite{PSSF}
  \item Test: using FZ tp estimate sigma and then testing integral.
\end{enumerate}
